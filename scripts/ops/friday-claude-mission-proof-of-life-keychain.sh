#!/usr/bin/env bash
#
# Provisioned-passphrase wrapper for friday-claude-mission-proof-of-life.sh.
#
# A previously provisioned macOS keychain item or owner-only passphrase file is
# streamed to the proof script over stdin. The passphrase is never stored in an
# environment variable, shell argv, or transcript.
#
set -euo pipefail
...
echo "Running Claude mission proof preflight (no passphrase, no traffic)..."
FRIDAY_CLAUDE_MISSION_PROOF_PREFLIGHT_ONLY=1 "${PROOF_SCRIPT}"
echo
