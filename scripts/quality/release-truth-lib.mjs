...
export function extractRequiresEnv(text) {
  const matches = text.match(/\b(?:FRIDAY|OPENAI|ANTHROPIC|OLLAMA|GOOGLE|XAI|GROQ|MISTRAL|DISCORD|TELEGRAM|LARK|FEISHU|WHATSAPP|SLACK|SMTP|OTEL|GRAFANA)_[A-Z0-9_]+\b/g) ?? [];
  return [...new Set(matches)].sort();
}

export function classifyReleaseProofEnvironment(requiresEnv, env = process.env) {
  const required = Array.isArray(requiresEnv) ? requiresEnv.filter((entry) => typeof entry === "string" && entry.length > 0) : [];
...
export function classifyEvidenceTarget(input) {
  ...
  const pathForMatch = `/${filePath}`;
  const content = input?.content ?? "";
  const text = [name, command, filePath, content].join("\n");
  const requiresEnv = extractRequiresEnv(text);
  const releaseProofEnvironment = classifyReleaseProofEnvironment(requiresEnv);

  const browserUiTarget = pathForMatch.includes("/test/e2e/ui/")
  ...
  const cloudTarget = pathForMatch.includes("/test/e2e/live/")
  ...
  const explicitMockTarget = pathForMatch.includes("/test/e2e/mock/")
    || pathForMatch.includes("/test/_mocks/")
  ...
  return {
    ...
    requiresEnv,
    releaseProofEnvironment: EVIDENCE_KINDS[evidenceKind].releaseProofEligible
      ? releaseProofEnvironment.status
      : "not_required",
    missingEnv: EVIDENCE_KINDS[evidenceKind].releaseProofEligible
      ? releaseProofEnvironment.missingEnv
      : [],
