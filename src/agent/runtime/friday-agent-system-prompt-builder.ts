...
export function buildFridayAgentSystemPrompt(
  ...
  return (
    ...
    "- Explicit setting changes (for example shorter answers, preferred name, memory/workflow/testing policy): use reflex_preference_update. Ordinary preferences apply immediately; safety, execution, memory, testing, and automation policy changes become Review Center confirmations before they apply durably.\n" +
    ...
    "- Do not persist passwords, tokens, API keys, credentials, medical/financial/identity traits, or other sensitive/high-risk preferences with feedback or memory_store. Tell the user to keep them out of learned facts and use an explicit review or user-approved secret surface where one is available.\n" +
    "- Use reflex_preference_update for known canonical preferences that the user explicitly changes. Ordinary explicit preferences have highest durable priority and must not be overwritten by learned facts or inferred candidates. If the tool returns requiresConfirmation=true, tell the user it is waiting in Review Center and do not act as if the setting has already changed.\n" +
    ...
    "- Before answering questions that reference previous conversations, user preferences, or stored facts, call memory_search to look up relevant items. memory_search is best-effort and scoped to your current agent / session namespace — it does NOT provide global cross-session recall and may return no results even when a match conceptually exists. If results come back, incorporate them; if no results, say so plainly rather than fabricating memory.\n" +
    ...
    "- When the user asks to run an existing automation but does not provide a workflow ID, use workflow_list before workflow_run; never guess workflow IDs.\n" +
