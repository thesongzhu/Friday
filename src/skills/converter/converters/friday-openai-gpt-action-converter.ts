...
function extractAuthInfo(spec: OpenApiSpec): AuthInfo {
  ...
  const seenAuthNames = new Map<string, number>();

  for (const [name, scheme] of Object.entries(securitySchemes)) {
    const safeAuthName = uniqueAuthReferenceName(name, seenAuthNames);
    const authRefs = {
      inputKey: `auth_${safeAuthName}`,
      envVarName: `AUTH_${safeAuthName.toUpperCase()}`,
    };
...
function uniqueAuthReferenceName(raw: string, seen: Map<string, number>): string {
  const base = sanitizeJsIdentifier(raw).replace(/^_+|_+$/g, "") || "scheme";
  const next = (seen.get(base) ?? 0) + 1;
  seen.set(base, next);
  return `${base}${next > 1 ? `_${String(next)}` : ""}`;
}

function assignOperationHandlerNames(
  operations: ExtractedOperation[],
): Array<{ op: ExtractedOperation; functionName: string }> {
  const seen = new Map<string, number>();
  return operations.map((op) => {
    const base = sanitizeJsIdentifier(op.operationId) || "operation";
    const next = (seen.get(base) ?? 0) + 1;
    seen.set(base, next);
    return {
      op,
      functionName: `op_${base}${next > 1 ? `_${String(next)}` : ""}`,
    };
  });
}
...
function buildAuthEnvVars(authInfo: AuthInfo): string[] {
  return authInfo.schemes.map((s) => s.envVarName);
}
...
function buildOperationIndexMjs(
  ...
  return `/**
 * Friday HTTP executor for: ${escapeGeneratedBlockCommentText(`${op.method} ${op.path}`)}
...
function buildCombinedIndexMjs(
  ...
  const handlers = assignOperationHandlerNames(operations);

  // Build per-operation handler functions with full path/query/header/body support
  const operationFunctions = handlers.map(({ op, functionName }) => {
    return `async function ${functionName}(inputs, env) {
  try {
    // Build URL with path parameters
    let url = ${JSON.stringify(baseUrl + op.path)};
    ${buildPathParamSubstitution(op)}
  ...
  const cases = handlers.map(({ op, functionName }) => {
    return `    case ${JSON.stringify(op.operationId)}:
      return await ${functionName}(inputs, env);`;
  }).join("\n");
...
function buildPathParamSubstitution(op: ExtractedOperation): string {
  ...
  return pathParams
    .map((p) => {
      const inputRef = `inputs[${JSON.stringify(p.name)}]`;
      return `if (${inputRef} === undefined || ${inputRef} === null) throw new Error(${JSON.stringify(`Missing required path parameter: ${p.name}`)});
    url = url.replace(${JSON.stringify(`{${p.name}}`)}, encodeURIComponent(String(${inputRef})));`;
    })
    .join("\n    ");
}
...
function buildQueryParamCode(op: ExtractedOperation): string {
  ...
  return queryParams
    .map((p) => `if (inputs[${JSON.stringify(p.name)}] !== undefined && inputs[${JSON.stringify(p.name)}] !== null) queryParams.set(${JSON.stringify(p.name)}, String(inputs[${JSON.stringify(p.name)}]));`)
    .join("\n    ");
}
...
function buildHeaderParamCode(op: ExtractedOperation): string {
  ...
  return headerParams
    .map((p) => `if (inputs[${JSON.stringify(p.name)}] !== undefined && inputs[${JSON.stringify(p.name)}] !== null) headers[${JSON.stringify(p.name)}] = String(inputs[${JSON.stringify(p.name)}]);`)
    .join("\n    ");
}
...
function buildAuthValueRefs(scheme: AuthInfo["schemes"][number]): {
  hasValue: string;
  value: string;
} {
  const inputRef = `inputs[${JSON.stringify(scheme.inputKey)}]`;
  const envRef = `env[${JSON.stringify(scheme.envVarName)}]`;
  return {
    hasValue: `${inputRef} || ${envRef}`,
    value: `${inputRef} ?? ${envRef}`,
  };
}
...
function buildAuthHeaderCode(authInfo: AuthInfo): string {
  ...
  for (const scheme of authInfo.schemes) {
    const auth = buildAuthValueRefs(scheme);
    if (scheme.type === "apiKey" && scheme.inLocation === "header" && scheme.headerName) {
      lines.push(`if (${auth.hasValue}) headers[${JSON.stringify(scheme.headerName)}] = ${auth.value};`);
    } else if (scheme.type === "http" && scheme.scheme === "bearer") {
      lines.push(`if (${auth.hasValue}) headers["Authorization"] = "Bearer " + (${auth.value});`);
    } else if (scheme.type === "http" && scheme.scheme === "basic") {
      lines.push(`if (${auth.hasValue}) headers["Authorization"] = "Basic " + (${auth.value});`);
    } else if (scheme.type === "oauth2") {
      lines.push(`if (${auth.hasValue}) headers["Authorization"] = "Bearer " + (${auth.value});`);
    }
  }
...
function buildAuthQueryCode(authInfo: AuthInfo): string {
  ...
  for (const scheme of authInfo.schemes) {
    ...
    if (scheme.type === "apiKey" && scheme.inLocation === "query" && scheme.headerName) {
      lines.push(`if (${auth.hasValue}) queryParams.set(${JSON.stringify(scheme.headerName)}, ${auth.value});`);
    }
  }
...
function buildAuthCookieCode(authInfo: AuthInfo): string {
  ...
  for (const scheme of authInfo.schemes) {
    ...
    if (scheme.type === "apiKey" && scheme.inLocation === "cookie" && scheme.headerName) {
      lines.push(`if (${auth.hasValue}) headers["Cookie"] = (headers["Cookie"] ? headers["Cookie"] + "; " : "") + ${JSON.stringify(scheme.headerName + "=")} + (${auth.value});`);
    }
  }
