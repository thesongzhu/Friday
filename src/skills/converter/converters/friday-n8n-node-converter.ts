...
function buildManifest(
  ...
  return {
    ...
    requirements: {
      bins: [],
      env: credentials.filter((c) => c.name).map((c) => toCredentialEnvVar(c.name!)),
      config: [],
...
function buildIndexMjs(descriptor: N8nNodeDescriptor, skillId: string): string {
  ...
  const credentialMapping = credentials
    .filter((c) => c.name)
    .map((c) => `      ${JSON.stringify(c.name!)}: { data: { apiKey: env[${JSON.stringify(toCredentialEnvVar(c.name!))}] ?? "" } },`)
    .join("\n");
  const safeImportSpecifier = isSafeN8nNodeImportSpecifier(nodeName);
  const importAttemptCode = safeImportSpecifier
    ? `try {
    ...
    : `return {
      result: {
        message: "n8n node import specifier was not safe — returning gathered parameters",
        node: ${JSON.stringify(nodeName)},
        parameters: params,
      },
      status: "success",
  ...
  return `/**
 * Friday adapter for n8n node: ${escapeGeneratedBlockCommentText(descriptor.displayName ?? descriptor.name ?? skillId)}
...
function toCredentialEnvVar(name: string): string {
  const normalized = name
    .toUpperCase()
    .replace(/[^A-Z0-9_]/g, "_")
    .replace(/_+/g, "_")
    .replace(/^_+|_+$/g, "");
  const safeName = normalized.length > 0 ? normalized : "CREDENTIAL";
  return `N8N_CRED_${/^[0-9]/.test(safeName) ? `_${safeName}` : safeName}`;
}

function isSafeN8nNodeImportSpecifier(name: string): boolean {
  if (!/^[A-Za-z0-9@._/-]+$/.test(name)) return false;
  if (name.startsWith(".") || name.startsWith("/") || name.includes("\\") || /^[A-Za-z][A-Za-z0-9+.-]*:/.test(name)) {
    return false;
  }
  return !name.split("/").some((part) => part === "." || part === ".." || part.length === 0);
}
