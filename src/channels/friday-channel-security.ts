...
const ENV_REF_REGEX = /^\$[A-Za-z_][A-Za-z0-9_]*$/;
...
export function parseFridayChannelSecretRef(raw: string): string | null {
  const value = raw.trim();
  if (!value.startsWith(FRIDAY_CHANNEL_SECRET_REF_PREFIX)) {
    return null;
  }
  const encoded = value.slice(FRIDAY_CHANNEL_SECRET_REF_PREFIX.length);
  if (!encoded) {
    return null;
  }
  try {
