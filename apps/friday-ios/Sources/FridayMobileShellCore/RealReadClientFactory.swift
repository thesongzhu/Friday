...
//
// PORTED from the desktop `FridayHubConsoleCore/RealReadClientFactory.swift` (#686). Builds the
// REAL `SealedWSReadClient` (the shared `FridayRustClient` package's sealed-WS read client)
// configured for the read-projection server's LOOPBACK host:port, with a concrete
// `NWConnection`-backed `SealedWSTransport`. This is the "the mobile shell actually reads the live
// Rust core" seam — the iOS mirror of the desktop live-read seam.
//
// LIVE ROUND-TRIP: `LoopbackSealedWSTransport` carries the FULL sealed-WS round trip — the
// cleartext length-prefixed preamble (`BE32(len)‖payload`), a hand-rolled RFC-6455 client
// WebSocket upgrade over the SAME raw `NWConnection` (masked client frames, the
...
// enrolled), the connect / preamble / handshake fails and surfaces as
// `FridayReadClientError.transport(...)` → the view model's honest "unavailable" — never fake-ready.
//
// NO DIVERGENT CRYPTO: this file reuses the shared package's `FridayCrypto.DeviceKeypair` (via
// `MasterKeyPeer`) + the shared `SealedWSReadClient` / `SealedWSTransport`; it only adds the
// `NWConnection` socket plumbing (byte-identical to desktop). iOS gains NO fresh crypto impl.
...
public struct ReadProjectionServerConfig: Sendable, Equatable {
  ...
  /// transport fails honestly → honest "unavailable".
...
/// (the session ends with no response).
...
public enum RealReadClientFactory {
  ...
  ///     / drive the negative control against the live server; real enrollment requires the
  ///     master-derived peer via `makeLive`).
  ...
  /// master key is missing/invalid — never substitutes an ephemeral key for the live path, and
  /// NEVER enrolls a fresh key (single-peer-trap safe: `MasterKeyPeer.deriveKeypair` only reads
  /// + derives in memory).
  ///
  /// This is the launch-mode the app selects under `FRIDAY_MOBILE_LIVE_READ=1` / `--live-read`.
  /// The honest-unavailable client stays the shipped default; this never runs without an enrolled
  /// host master key, and only on the env/arg opt-in.
  ...
  /// existing `HonestlyUnavailableReadClient` (defined in HomeViewModel.swift) — no duplicate.
