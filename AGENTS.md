...
- Do not silently invoke owner/admin branch-protection bypass. Both (i) explicit `--admin` flag merges and (ii) the implicit admin/owner merge route enabled when `enforce_admins.enabled: false` on the protected branch are forbidden by default. The single-maintainer owner-bypass exception is defined in `/Users/jarvis/Desktop/friday_master_workflow_prompt.md` §23.6 (master prompt is repo-external local operator policy; this repo-tracked rule mirrors the exception's hard requirements for in-repo durable visibility). The exception is per-merge, narrow, never standing, and never covers safety / security / release-governance / irreversible action / approval boundary / secrets / tokens / runtime execution / memory or context-compression semantics / provider / plugin / MCP / skill lifecycle / default-on behavior / release-proof standard. You may draft or open a PR even when owner-bypass may be needed to land it, but only if the PR body includes the required owner-bypass checklist mirroring the master prompt §23.6 pre-merge capture conditions. You must not recommend, assist with, or proceed to the merge action unless every §23.6 owner-bypass condition is satisfied AND the per-merge approval is captured durably in a pre-merge PR-side artifact (PR body section OR PR comment) BEFORE the merge action. Merge action alone is never approval; an in-thread statement that was not captured durably before merge is not retroactive approval. Post-merge records (the post-merge ledger update, the F-013 entry) document what happened; they cannot retroactively supply a pre-merge approval that was never captured. Owner-bypass is governance bookkeeping, not release proof, and does not weaken the §6 / Stage 5 two-isolated-reviewer requirement.
...
- Owner/admin branch-protection bypass merge is being contemplated — i.e. the PR author is also a repo admin, branch protection requires a review the user cannot satisfy non-admin, and the only way to land the PR is the implicit `enforce_admins=false` bypass (or, equivalently, an explicit `--admin` flag). In that case Claude must: (a) state the facts explicitly and quote the platform-state field that allows the bypass; (b) ask whether the master prompt §23.6 single-maintainer owner-bypass exception applies; (c) require the user to confirm the scope is not safety / security / release-governance / irreversible / approval / secrets / runtime / memory / provider / plugin / MCP / skill / default-on / release-proof, and confirm every other §23.6 condition is met; (d) require the user's per-merge approval line to be captured in a durable PR-side artifact (PR body section or PR comment) BEFORE the merge action; (e) record the verbatim approval line, the stated reason no second write-access reviewer is available, CI status, RGG artifact status framed honestly, and the two-reviewer PASS trail in the post-merge ledger and the F-013 governance entry. Records that exist only after the merge action are not retroactive approvals.
...
- The master prompt at `/Users/jarvis/Desktop/friday_master_workflow_prompt.md` is **repo-external local operator policy**; it is not under repo version control. Governance / hard-rule changes made to the master prompt have no in-repo durable audit trail unless they are mirrored into a repo-tracked doc (this `AGENTS.md`, `context/AGENTS.md`, or a repo-tracked finding in `docs/audit/10_FINDINGS_REGISTER.md`). Repo-external policy changes are not release proof, are not visible to PR reviewers from the repo alone, and must not appear in any PR diff. When a master prompt change has governance impact, the same slice must land the in-repo durable mirror as part of its PR diff (e.g. the owner-bypass exception added to master prompt §23.6 is mirrored in this AGENTS.md hard rule + the F-013 row in `docs/audit/10_FINDINGS_REGISTER.md`).
