...
import { hashPasswordScrypt } from "#api";

/** Default test passphrase for the local test user. */
export const TEST_USER_PASSPHRASE = "any";
...
export function createTestDb(): FridaySqliteLayer {
  ...
  // Insert a test user with password hash for FK constraints and auth tests
  const passwordHash = hashPasswordScrypt(TEST_USER_PASSPHRASE);
  db.prepare(
