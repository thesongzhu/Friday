...
# Phase 8 Code Review Package

## Test Results
```
117 test files, 1136 tests, ALL PASSED
```

## Design Doc: docs/phase8-implementation-plan.md
...
export function createFridayAuthMiddlewareFactory(
  ...
  return {
    ...
    enforceRateLimit(ctx, policyId) {
      const key = getRateLimitKey(ctx, policyId);
    ...
    login(request, ip, userAgent) {
      ...
      if (request.localPassphrase) {
        ...
        if (
          request.password &&
          user.password_hash &&
          !verifyPassword(request.password, user.password_hash)
        ) {
...
export function createFridayWorkflowConflictService(
  ...
  return {
    ...
    resolveConflict(conflictId, request, resolvedByUserId) {
      ...
      const resolved = deps.db.withWriteTransaction((db) => {
        return repo.resolve(db, conflictId, resolvedByUserId, now);
      });
      ...
      // Build a mock draft response representing the resolved state
      // In production, this would update the actual draft based on the resolution strategy
...
export function createFridayRealtimeRoutes(
  ...
  return [
    ...
    },
    {
      ...
      async handler(ctx) {
        const { streamId, afterSeq, limit } = ctx.body as FridayRealtimePullRequest;
...
import { isFridayLegacyWriteFrozen } from "./friday-legacy-write-freeze-guard.js";
...
export function createFridayRealtimeEventBus(
  ...
  return {
    publish<TEvent extends FridayRealtimeEventName>(
      ...
      // Persist if persistence callback is provided
...
export const V002_PHASE8_API_FOUNDATION_MIGRATION: FridaySqliteMigration = {
  ...
  checksum: computeFridayMigrationChecksum(V002_PHASE8_API_FOUNDATION_SQL),
};
```
...
describe("FridayLegacyDecommissionService", () => {
  ...
  it("verification succeeds when guard is also activated", () => {
    // The service's freezeLegacyWrites() sets its own state,
    // but verifyNoLegacyWrites checks isFridayLegacyWriteFrozen() from the guard module
    service.freezeLegacyWrites();
    // Must also activate the guard module
    activateFridayLegacyWriteFreeze();
    const result = service.verifyNoLegacyWrites("2025-06-15T09:00:00.000Z");
    expect(result.ok).toBe(true);
