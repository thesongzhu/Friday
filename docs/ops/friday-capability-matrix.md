...
also closes Telegram/Discord/Lark workflow-candidate approve/reject live proof
for Phase24E/F/G on GitHub main, with the LLM bridge explicitly stubbed, and
reduces unnecessary DeepSeek spend on push RGG runs. Phase24H natural-trigger
execution remains paused / `proof_pending`. Package/npm truth stays separate.
Do not read these source closures as already published npm `1.0.2` behavior.
