...
| Skill discovery + manifest loader + lifecycle | `/v1/skills/*` | CLAUDE_SKILL_TESTS unit/integration (3 tests in run-real-green-gate.mjs) | `mock-only` (unit/integration) |
...
| Self-healing executors (`disable_skill`, `retry_node`, `switch_model_fallback`, `trim_payload`, `pause_workflow`) | hub-wired side effects | `friday-self-healing-live` covers the auto-fix path overall; per-executor matrix not enumerated this audit | `local real-runtime + real-provider <openai\|deepseek>` for the holistic test |
