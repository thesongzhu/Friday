...
- Dirty workspace `npm run check:migrations` fails only because untracked duplicate migration files such as `v038-retired-feature-reserved-a 3.ts` are present. A temporary copy containing only Git-tracked files passed the same script.
...
| DATA-004 | P2 | Dirty local duplicate files can break filesystem-scanning gates. | `npm run check:migrations` failed on untracked `* 3.ts` migration copies. | Workspace hygiene issue, not branch issue. | Remove/quarantine local duplicate files outside this branch or run checks from clean worktree. |
