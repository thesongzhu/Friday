...
- The public v1 local track is limited to local UI + operator + non-technical independent-use readiness. Its release-facing claims must not exceed current proof: configured trusted-channel inbound for Discord/Telegram/Lark+Feishu is proven on the `1.0.2` release SHA, and GitHub-visible main after `1.0.2` also proves Phase24E/F/G workflow-candidate approve/reject with an explicitly stubbed LLM bridge. Those later channel-candidate proofs are not npm `1.0.2` truth. Phase24H natural-trigger execution, live LLM generation, channel/cloud release-complete proof, external OTEL/Grafana proof, and release-complete-all remain explicit future/no-claim items unless a later phase provisions the required external environment and validates same-SHA RGG truth.
...
- Live external-channel proof is per-channel and scoped to configured test spaces only. The `1.0.2` release SHA proves configured trusted-user inbound receipt for Discord, Telegram, and Lark/Feishu. GitHub-visible main after `1.0.2` additionally proves workflow-candidate approve/reject for Telegram, Discord, and Lark/Feishu, with the LLM bridge explicitly stubbed. Docs and UI copy must not stretch those receipts into outbound channel automation, live LLM generation, natural-trigger workflow execution, or package truth for npm `1.0.2`. `blocked_by_env` is not pass.
