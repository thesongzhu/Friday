...
- Public-web recommendation proof is currently bounded to Friday-owned controlled-fixture flows: focused `/v1/agent/runs` live DeepSeek harnesses have fetched public URL fixtures with `web_fetch`, returned sourced shadow-review-only recommendations, rejected hostile/sponsored sources, and left poison markers out of durable memory/preference/learning/world-model tables; one targeted real-browser `/chat` flow proves the same shape for four controlled public URLs. This does not prove a dedicated public-web discovery/recommendation route, broad live-web crawl, durable recommendation store, third-party authenticated browsing, install/promote automation, or exhaustive public-web UI coverage.
