...
- `/v1/packages*` and `/v1/security/tenants*` are code-present but env-gated. They are only active on runtimes started with `FRIDAY_PACKAGING_ENABLED=true` or `FRIDAY_MULTI_TENANT_ENABLED=true`. When either is enabled, the routes activate an in-memory engine only. For `FRIDAY_PACKAGING_ENABLED=true`, package registry, install, lifecycle, and trusted-key state does not persist across hub restarts and the bootstrap publish handler currently uses stub manifest data — real signature verification is not wired through that path. For `FRIDAY_MULTI_TENANT_ENABLED=true`, Friday's per-domain repositories (sessions, skills, workflows, providers, memory, rules) do not persist or scope by tenant. Neither path is a persistence-level guarantee and neither must be presented as one.
...
- `/v1/skills/*` is the intended canonical skill lifecycle surface for catalog, detail, install, update, delete, manifest validation, and verification. This is design intent; the standalone hub does **not** currently register the full set (see Finding F-017 in [`docs/audit/10_FINDINGS_REGISTER.md`](audit/10_FINDINGS_REGISTER.md)).
- Standalone hub currently registers only the always-on subset of `/v1/skills/*`: `GET /v1/skills` (inventory list), `POST /v1/skills/:skillId/run`, and `PATCH /v1/skills/:skillId/content` (managed-skills-dir + canonical-approval gated). The lifecycle-gated routes — `GET /v1/skills/catalog`, `GET /v1/skills/:skillId`, `POST /v1/skills/validate-manifest`, `POST /v1/skills/:skillId/verify`, and the lifecycle-branch `install`/`update`/`delete` — are not registered because hub bootstrap does not construct `createFridaySkillLifecycleService` nor pass `skillLifecycle` into `createFridayApiRuntime`. A request against an unregistered route returns HTTP 404 `No route matches` from the router, not from `verifySkill` or any lifecycle service handler.
- Lifecycle detail, catalog, validate-manifest, and verify are blocked from standalone-hub HTTP coverage until `skillLifecycle` is wired in a separate slice. Unit/contract tests at `mock-only` proof tier exist (e.g. `test/unit/api/http/routes/friday-skill-routes.test.ts`, `test/unit/api/runtime/friday-api-runtime-extra-route-registration.test.ts`) and use mock lifecycle deps; they are not real-runtime evidence.
- The lifecycle-branch `update` and `delete` routes cannot be wired alongside the lifecycle service until canonical approval gating is added. Neither the lifecycle service nor the lifecycle-branch routes currently call `assertCanonicalApproval`; only `PATCH /v1/skills/:skillId/content` enforces it today. Wiring lifecycle before that approval gate exists would expose unapproved mutating skill routes and is blocked by §3 ask-before-act on safety boundaries.
