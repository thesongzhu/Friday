...
- `/v1/packages*` and `/v1/security/tenants*` are code-present but env-gated and default-off. They are only active on runtimes started with `FRIDAY_PACKAGING_ENABLED=true` or `FRIDAY_MULTI_TENANT_ENABLED=true`. Phase 11 (PR #233, merged `a5239ac7`) wired SQLite-backed persistence behind both gates: `src/packaging/persistence/friday-package-sqlite-store.ts` persists package registry, install lifecycle, rollback history, lifecycle audit, and trusted-key state, and the bootstrap publish handler now invokes real `verifySignatureLogical` against the trusted-key store rather than stub manifest data; `src/security/multi-tenant/persistence/friday-multi-tenant-sqlite-store.ts` persists tenant, secret, audit, and tenant-scoped resource registry rows. Phase 11 closed as `partial` and forwarded named Phase 14 release-proof debt — `module_16_packaging_release_proof_roundtrip`, `module_17_full_upgrade_lifecycle_evidence_harness`, and `module_18_cross_tenant_denial_rgg_assertion` — so persistence-shipped is not the same as default-on, release-proof, or end-to-end cross-tenant denial closure. `FRIDAY_PACKAGING_ENABLED=true` and `FRIDAY_MULTI_TENANT_ENABLED=true` default-on flips remain explicit stop points.
