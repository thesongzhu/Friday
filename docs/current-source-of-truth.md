...
- `/v1/packages*` and `/v1/security/tenants*` are code-present but env-gated. They are only active on runtimes started with `FRIDAY_PACKAGING_ENABLED=true` or `FRIDAY_MULTI_TENANT_ENABLED=true`. When `FRIDAY_MULTI_TENANT_ENABLED=true`, the routes activate an in-memory engine only — Friday's per-domain repositories (sessions, skills, workflows, providers, memory, rules) do not persist or scope by tenant, so this path is not a persistence-level isolation guarantee and must not be presented as one.
