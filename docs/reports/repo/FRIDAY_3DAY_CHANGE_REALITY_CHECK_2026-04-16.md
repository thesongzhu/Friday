...
| skill-run-route | 技能执行 | /v1/skills/:skillId/run 已真实打通。 | partially verified | skillId=autofix-readiness-review, status=200, returnedStatus=n/a. | Live POST /v1/skills/:skillId/run against the first installed skill. Current route proves dispatch acceptance, not full skill execution. |
...
| packaging-runtime | PR #130 / 打包 API | /v1/packages/* 现在默认就是当前 runtime 的公开能力。 | verified | /v1/packages status=200; current runtime only wires packaging when FRIDAY_PACKAGING_ENABLED=true. | Live GET /v1/packages plus src/hub/friday-hub-bootstrap.ts gate. |
| multi-tenant-runtime | 安全 / 多租户 | 多租户 runtime surface 当前已经默认可用。 | verified | /v1/security/tenants status=200; current runtime only wires tenant routes when FRIDAY_MULTI_TENANT_ENABLED=true. | Live GET /v1/security/tenants plus src/hub/friday-hub-bootstrap.ts gate. |
...
| self-healing-loop | 自我修复 | 自我修复闭环已经被真实打通到 execute/verify/rollback。 | partially verified | /v1/auto-fix/actions status=200. Inventory/readiness is reachable, but the current proof pack still does not prove a full live execute -> verify -> rollback loop. | Live GET /v1/auto-fix/actions plus proof-pack review. |
| compaction-proof | PR #129 / 语义压缩 | compaction 已经被真实证明会触发、写入 memory，并被后续 run 读回。 | not proven | Code and tests exist, but the current active proof pack does not yet include a live compaction trigger/readback artifact. | Proof-pack gap analysis against the active real validation suite. |
| autonomous-persistence-proof | PR #132 / 自主引擎 SQLite 持久化 | autonomous persistence 已被真实证明可跨重启恢复。 | not proven | SQLite repository code and unit tests exist, but the current active proof pack does not yet include a live restart/recovery artifact. | Proof-pack gap analysis against the active real validation suite. |
