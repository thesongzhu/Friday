...
| skills-marketplace-readiness | 技能 / 市场 | 技能目录、source、marketplace assets 已达到可公开浏览状态。 | partially verified | /v1/skills=70, /v1/skills/catalog=0, /v1/marketplace/sources=0, /v1/marketplace/assets=0. | Live runtime inventory and marketplace probes. |
| skill-run-route | 技能执行 | /v1/skills/:skillId/run 已真实打通。 | verified | skillId=ai-inference, probeKind=built-in, status=200, returnedStatus=completed, completionDepth=executed. | Live POST /v1/skills/ai-inference/run against a real provider-backed runtime. Route must return completed + executed, not dispatch-only acceptance. |
...
| packaging-runtime | PR #130 / 打包 API | /v1/packages/* 现在默认就是当前 runtime 的公开能力。 | blocked-by-env | /v1/packages status=200; current runtime only wires packaging when FRIDAY_PACKAGING_ENABLED=true. | Live GET /v1/packages plus src/hub/friday-hub-bootstrap.ts gate. |
| multi-tenant-runtime | 安全 / 多租户 | 多租户 runtime surface 当前已经默认可用。 | blocked-by-env | /v1/security/tenants status=200; current runtime only wires tenant routes when FRIDAY_MULTI_TENANT_ENABLED=true. | Live GET /v1/security/tenants plus src/hub/friday-hub-bootstrap.ts gate. |
...
| search-latestness | 搜索 | 当前 runtime 的 search freshness 已经被真实验证。 | verified | /v1/health capabilities.search.latestness=provider_backed; docs/reports/repo/FRIDAY_FINAL_REAL_PROOF_PACK_2026-04-16.md also contains a live MCP dated-query proof with time-bounded results. | Live /v1/health capability snapshot plus latest final proof-pack MCP dated-query evidence. |
